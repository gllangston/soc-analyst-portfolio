...
I then took the file hash and checked it in VirusTotal, where multiple vendors flagged it as malicious and tied it to a known exploit, confirming the file wasn’t safe. 

After that, I wanted to see if the system was actually impacted, so I looked at the behavior and logs (Wazuh). I then ran the file in a sandbox environment (AnyRun), that’s when I saw that PowerShell was triggered, and the machine-made outbound connections to a known malicious IP, which isn’t normal for a Word document. 
